...
QEMUFLAGS_GENERAL   := -kernel $(KERNEL_IMAGE) -serial mon:stdio -nographic -no-reboot \
                       -m 3G -smp cpus=$(NCPU),cores=1,threads=1,sockets=$(NCPU) \
                       -netdev user,id=vmnic -device virtio-net,netdev=vmnic
...
start: $(KERNEL_IMAGE) $(INITRAMFS_IMAGE)
	$(QEMU) $(QEMUFLAGS_ROOTDISK) $(QEMUFLAGS_GDB)
#	$(QEMU) $(QEMUFLAGS_INITRAMFS) $(QEMUFLAGS_GDB)
.PHONY: start

attach:
	gdb kernel/vmlinux -ex "target remote :$(GDBPORT)" -x "gdb/config.gdb"
.PHONY: attach
...
raspi-kgdb-agent:
	./scripts/agent-proxy/agent-proxy 5550^5551 0 /dev/ttyUSB0,115200
