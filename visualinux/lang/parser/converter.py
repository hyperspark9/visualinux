...
from visualinux.lang.parser.units import *
from visualinux.lang.parser.utils import *

class Converter:

    def convert(self, parsetree: ParseTree) -> list[Instruction]:
    ...
    def add_typedef(self, name: str, typedef: BoxDecl | ContainerDecl) -> None:
    ...
    # ======================================================================
    # handle the top-level instruction: viewdef
    # ======================================================================

    def parse_viewdef(self, inst: Tree[Token]) -> ViewDef:
        assert inst.data == 'viewdef'
        name = serialize(inst.children[0])
        viewdef = ViewDef(name, [])
        for item in scan_children_as_tree(inst, skip=1):
            match item.data:
                case 'plot': viewdef.plots.append(self.parse_plot(item))
                case 'comment': pass
    ...
    def parse_box_decl(self, typename: str, node: Tree[Token]) -> BoxDecl:
        ...
        typo  = self.get_term_from(node_typo) if node_typo else None
        absts = self.parse_abst_decl_body(child_as_tree(node, 1))
        node_where = child_as_tree(node, 2)
        decl = BoxDecl.creat_in_typedef(typename, typo, absts)
        self.add_typedef(typename, decl)
        if node_where:
    ...
    def parse_box_def(self, node: Tree[Token]) -> BoxDef:
        ...
        abst  = self.parse_abst_def_body(child_as_tree(node, 2))
        node_where = child_as_tree(node, 3)
        where = self.parse_where_block(node_where) if node_where else None
        return BoxDef.creat_in_place(root, typo, label, [abst], where)
    ...
    # ======================================================================
    # handle the abst inside box
    # ======================================================================

    def parse_abst_decl_body(self, body: Tree[Token]) -> list[AbstDef]:
        assert body.data == 'abst_decl_body'
        if child_as_tree(body, 0).data == 'abst_def_body':
            return [self.parse_abst_def_body(child_as_tree(body, 0))]
        abstractions = []
        for abst in scan_children_as_tree(body):
            if abst.data == 'comment': continue
            assert abst.data == 'abst_definition'
            abstractions.append(self.parse_abst_def(abst))
        return abstractions

    def parse_abst_def(self, abst: Tree[Token]) -> AbstDef:

        assert abst.data == 'abst_definition'

        abst_body = child_as_tree(abst, 1)
        abst_def  = self.parse_abst_def_body(abst_body)

        abst_id   = child_as_tree(abst, 0)
        if abst_id.data == 'abst_identifier':
            abst_def.name   = serialize(child_as_tree(abst_id, 0))
        else:
            abst_def.parent = serialize(child_as_tree(abst_id, 0))
            abst_def.name   = serialize(child_as_tree(abst_id, 1))

        return abst_def

    def parse_abst_def_body(self, body: Tree[Token]) -> AbstDef:

        assert body.data == 'abst_def_body'
        abst_def = AbstDef('default', None, [], set())

        node_insts = child_as_tree(body, 0)
        for node in scan_children_as_tree(node_insts):
            assert node.data == 'abst_def_inst'
            abst_def.insts += self.parse_abst_inst_seq(node)

        if node_distillers := child_as_tree(body, 1):
            for node in scan_children_as_tree(node_distillers):
                assert node.data == 'distiller'
                abst_def.distillers.add(self.parse_distiller(node))

        return abst_def

    def parse_abst_inst_seq(self, node: Tree[Token]) -> list[AbstInst]:
        assert node.data == 'abst_def_inst'
        inst = child_as_tree(node, 0)
        insts: list[AbstInst] = []
        match inst.data:
            ...
            case 'abst_operation':
                insts.append(self.parse_abst_opt(inst))
            case 'comment':
                pass
            case _:
                raise fuck_exc(UnexpectedTokenError, inst)
    ...
    def parse_abst_opt(self, inst: Tree[Token]) -> AbstOpt:
        assert inst.data == 'abst_operation'
