...
# ======================================================================
# Abstraction classes to eliminate common fields/methods.
...
# ======================================================================
# box definition, abst and where block
# ======================================================================
...
class BoxDecl(ShapeDecl):
    ...
    def __init__(self, name: str, term: Term | None,
                 body: list[AbstDef], where: WhereBlock | None, recursion: bool = False) -> None:
    ...
    @classmethod
    def creat_in_typedef(cls, name: str, term: Term | None, body: list[AbstDef]) -> 'BoxDecl':
...
class BoxDef(ShapeDef):
    ...
    def creat_in_place(cls, root: Term | None, term: Term | None, label: str,
                       body: list[AbstDef], where: WhereBlock | None) -> 'BoxDef':
...
class ViewDef:
    ...
    def format_string(self, depth: int = 0) -> str:
        lines = [padding(depth) + f'view {self.name} {{']
        for plot in self.plots:
            lines.append(plot.format_string(depth + 1))
        lines.append(padding(depth) + '}')
        return '\n'.join(lines)
        
Instruction = Assignment | ViewDef
AbstInst = TextDef | LinkDef | InnerShapeDef | BoxDef | ContainerDef | AbstOpt
