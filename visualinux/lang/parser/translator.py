...
class Translator:
    ...
    def __init__(self) -> None:
        self.model = View() # list of (shape, abst)

    def translate(self, insts: list[Instruction]) -> View:
        ...
        for inst in insts:
            ...
            elif isinstance(inst, ViewDef):
                self.interpret_viewdef(inst)
            else:
    ...
    # ======================================================================
    # top-level translation (assignment/focus/viewdef)
    # ======================================================================
    ...
    def interpret_viewdef(self, viewdef: ViewDef):
        plots: list[PlotTarget] = []
        for i, target in enumerate(viewdef.plots):
            name = f'{viewdef.name}.plot#{i}'
            plots.append(self.interpret_plot(name, target))
        self.model.subviews[viewdef.name] = plots
    ...
    def interpret_box(self, varname: str, shapedef: BoxDef, parent_shape: Box | Container | SwitchCase) -> Box:
        ...
        if shapedef.decl.term:
            label = label or str(shapedef.decl.term.head) 
        elif root:
        ...
        for abstdef in shapedef.body:
            abst = Abst(abstdef.name, abstdef.parent, OrderedDict(), abstdef.distillers, box)

            for inst in abstdef.insts:

                if vl_debug_on(): printd(f'----[DEDBUG] intp_box {label=} {abstdef.name=} {inst=!s}')
                if isinstance(inst, AbstOpt):
                    if vl_debug_on(): printd(f'>> abst_opt not handleed now..')
                    continue # TODO. abst impl abst opt

                if inst.label in used_labels or inst.label in abst.members:
                    raise fuck_exc(AssertionError, f'interpret_box duplicated key {inst.label} in {shapedef.format_string_head()}')

                if isinstance(inst, TextDef):
                    assert inst.root
                    member = Text(inst.label, inst.root, inst.typo, abst)
                    ...
                    member = Link(inst.label, abst, inst.link_type, target)
                    ...
                    raise fuck_exc(AssertionError, f'interpret_box unknown {inst = } in {abstdef.name = }')

                abst.members[inst.label] = member

            # we don't allow the same label appears in different absts of the same box
            for label in abst.members:
                used_labels.add(label)

            box[abstdef.name] = abst
