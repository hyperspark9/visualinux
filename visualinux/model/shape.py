...
class PrimitiveShape(Shape):
    ...
    def __init__(self, name: str, label: str, root: Term, type: Term | None, parent_abst: 'Abst') -> None:
        super().__init__(name, label, root, type)
        self.parent_abst = parent_abst

    @property
    def parent_box(self) -> 'Box':
        return self.parent_abst.parent_box
    @property
    ...
    @abstractmethod
    def clone_to(self, parent_abst: 'Abst') -> 'PrimitiveShape':
        pass
...
class Text(PrimitiveShape):
    ...
    def __init__(self, label: str, root: Term, typo: TextFormat, parent_abst: 'Abst') -> None:
        super().__init__('Text', label, root, None, parent_abst)
        self.typo = typo
        self.root: Term

    def clone_to(self, parent_abst: 'Abst') -> 'Text':
        return Text(self.label, self.root, self.typo, parent_abst)
...
class Link(PrimitiveShape):
    ...
    def __init__(self, label: str, parent_abst: 'Abst', link_type: LinkType,
                 target: 'PrimitiveShape | Box | Container | SwitchCase | ContainerConv | None' = None) -> None:
        super().__init__('Link', label, Term.CExpr('NULL'), None, parent_abst)
    ...
    def clone_to(self, parent_abst: 'Abst') -> 'Link':
        if self.target is not None:
            if vl_debug_on(): printd(f'[DEBUG] Link clone_to parent={parent_abst.parent_box.format_string_head()}')
            assert isinstance(self.target, NotPrimitive | ContainerConv)
            new_target = self.target.clone_to(parent_abst.parent_box)
        else:
            new_target = None
        return Link(self.label, parent_abst, self.link_type, new_target)
...
class Abst:
    ...
    def clone_to(self, parent_box: 'Box') -> 'Abst':
        new_abst = Abst(self.name, self.parent, OrderedDict(), self.distillers, parent_box)
        for label, member in self.members.items():
            if isinstance(member, PrimitiveShape):
                new_member = member.clone_to(new_abst)
            else:
                assert isinstance(member, NotPrimitive)
                new_member = member.clone_to(new_abst.parent_box)
            new_abst.members[label] = new_member
        return new_abst

    def evaluate_on(self, pool: Pool, item_value: KValue | None = None, distillers: set[Distiller] | None = None) -> entity.Abst:

        distillers = distillers or set()
        for distiller in self.distillers:
            distillers.add(distiller)
        if vl_debug_on(): printd(f'Abst {self.name} {distillers = !s}')
        distilled = self.evaluate_distillers(distillers, item_value)
        if vl_debug_on(): printd(f'Abst {self.name} {distilled = !s}')

        members = OrderedDict()
        if vl_debug_on(): printd(f'abst {self.name} eval {item_value = !s}')
        for sub, member in self.members.items():
            if vl_debug_on(): printd(f'abst {self.name} _eval {sub}: {member.format_string_head()} of {self.parent_box.format_string_head()}')
        ...
        return entity.Abst(self.name, self.parent, members, distilled)

    def evaluate_distillers(self, distillers: set[Distiller], item_value: KValue | None = None) -> bool:
        if vl_debug_on(): printd(f'Abst {self.name} eval {distillers = !s}')
        ...
        for distiller in distillers:
            ...
            if cond == True:
                ...
                distilled = True
        # return show?
        if vl_debug_on(): printd(f'Abst {self.name} => {distilled = }, {has_distiller = }')
...
class Box(Shape):
    ...
    def __init__(self, name: str, label: str, root: Term | None, type: Term | None,
                 absts: OrderedDict[str, Abst], parent: 'NotPrimitive') -> None:
        super().__init__(name, label, root, type)
        self.absts  = absts
        self.parent = parent
    ...
    def __contains__(self, key: str): return self.absts.__contains__(key)
    def __getitem__(self, key: str): return self.absts.__getitem__(key)
    def __setitem__(self, key: str, value: Abst): self.absts.__setitem__(key, value)
    ...
    def format_string(self, depth: int = 0) -> str:
        ...
        for abst in self.absts.values():
            lines.append(abst.format_string(depth + 1))
        return '\n'.join(lines)
    ...
    def evaluate_on(self, pool: Pool, item_value: KValue | None = None, distillers: set[Distiller] | None = None) -> entity.Box:
        ...
        if root.value == KValue_NULL.value:
            ...
            return entity.Box(self, root, self.label, OrderedDict({'default': entity.Abst('default', None, OrderedDict(), distilled=True)}))

        label = self.scope.demix_label(self.label)
        ent = entity.Box(self, root, label, OrderedDict())
        pool.add_box(ent)

        if vl_debug_on(): printd(f'Box evaluate_on {root=!s} {self.absts=!s}')
        if isinstance(root, KValueXBox):
            if vl_debug_on(): printd(f'!!XBOX {self=!s}')
        for abst in self.absts.values():
            ent.absts[abst.name] = abst.evaluate_on(pool, item_value, distillers)
    ...
    def clone_to(self, parent: 'NotPrimitive') -> 'Box':
        ...
        for name, abst in self.absts.items():
            new_box[name] = abst.clone_to(new_box)
        return new_box
    ...
    def find_member_of(self, field_seq: list[str]) -> 'PrimitiveShape | Box | Container | SwitchCase | ContainerConv':
        ...
        for field in field_seq:
            ...
            for abst in member.absts.values():
                if field in abst.members:
                    member = abst[field]
                    break
        return member

    # def add_member(self, abst, member: Shape):
    #     pass
...
class BoxRecursion(Box):
    ...
    def expand_to(self, parent_shape: 'NotPrimitive') -> Box:
        ...
        for name, abst in self.origin_shape.absts.items():
            if vl_debug_on(): printd(f'    clone abst {name}')
            new_box[name] = abst.clone_to(new_box)
