...
class Array(Container):
    ...
    def evaluate_on(self, pool: Pool, iroot: KValue | None = None, distillers: set[Distiller] | None = None) -> entity.Container:
        ...
        if not root.is_pointer():
            root = root.address_of()
        # root = root.cast(self.member_shape.type)
        if vl_debug_on(): printd(f'    {root = !s}')
        if vl_debug_on(): printd(f'    {root.inner = !s}')
        if vl_debug_on(): printd(f'    {root.inner.dereference() = !s}')
        ...
        if vl_debug_on(): printd(f'    {root.type.name = !s}')
        if vl_debug_on(): printd(f'    {root.type.array_length() = !s}')
