            ...
            for abst in ent.absts.values():
                for member in abst.members.values():
                    if isinstance(member, entity.BoxMember):
                        ...
                        if vl_debug_on(): printd(f':{abst.name} set_parent {ent_child.key=} .parent= {key=}')
        for key, ent in self.pool.containers():
...
class State:
    ...
    def __init__(self) -> None:
        self.subviews: dict[str, Subview] = {}

    def to_json(self) -> dict:
        return dict((name, subview.to_json()) for name, subview in self.subviews.items())
