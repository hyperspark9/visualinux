...
function PrimaryWindowHeader({ wKey, diagramRef, onMount }: {
    ...
    let buttons: ButtonDef[] = useMemo(() => [{
        ...
        onClick: () => {
            let objectKey = model.getObjectSelected(wKey);
            if (viewDisplayed !== undefined && objectKey !== undefined) {
                stateDispatch({ type: 'PICK', viewDisplayed, objectKey });
            }
        },
        ifEnabled: model.getObjectSelected(wKey) !== undefined,
        icon: "share",
        ...
        onClick: () => {
            ...
            stateDispatch({ type: 'REMOVE', wKey });
        },
...
function SecondaryWindow({ node }: { node: SecondaryPanel }) {
    ...
    let buttons: ButtonDef[] = [{
        onClick: () => {
            ...
            stateDispatch({ type: 'FOCUS', objectKey: node.objectKey });
        },
        ifEnabled: true,
        icon: "bookmark",
        desc: "focus"
    ...
    //     onClick: () => {
    //         // TODO;
    //         let objectKey = model.getObjectSelected(node.key, false);
    //         let viewDisplayed = node.viewDisplayed;
    //         if (viewDisplayed !== undefined && objectKey !== undefined) {
    //             stateDispatch({ type: 'PICK', viewDisplayed, objectKey });
    //         }
    //     },
    ...
    // }, {
        onClick: () => {
            ...
            stateDispatch({ type: 'ERASE', wKey: node.key });
        },
        ifEnabled: true,
        icon: "delete",
        desc: "remove"
