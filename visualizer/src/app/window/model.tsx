...
export class WindowModel {
    ...
    split(key: number, direction: SplitDirection) {
        ...
        console.log(`[DEBUG] before split`, this.toString(), direction);
        node.split(direction);
        console.log(`[DEBUG] after split`, this.toString());
    }
    remove(key: number) {
        console.log(`[DEBUG] before remove`, this.toString());
        let node = this.find(key);
        if (node === undefined) {
            throw new Error(`window.remove(): failed to find window with key=${key}.`);
        }
        ...
        while (p.parent != null) {
            // console.log(`[DEBUG] for p`, p.children[0].toString(), p.parent.toString());
            if (p.children.length == 1) {
                p.parent.replaceChild(p, p.children[0]);
            }
            p = p.parent;
        }
        console.log(`[DEBUG] after remove`, this.toString(), this.root);
    }
    pick(viewDisplayed: string, objectKey: string) {
        console.log(`[DEBUG] pick`, viewDisplayed, objectKey);
